...
async function upsert(rows) {
  ...
  const values = rows.map(e => `(${esc(e.id)},${esc(e.created_by)},${esc(e.date_added)},${esc(e.start_time)},${esc(e.status)},${esc(e.calendar_id)},${esc(LOC)},${e.deleted ? 'true' : 'false'})`).join(',');
  const sql = `insert into ghl_appointments (id, created_by_user_id, date_added, start_time, status, calendar_id, location_id, deleted)
    values ${values}
    on conflict (id) do update set created_by_user_id=excluded.created_by_user_id, date_added=excluded.date_added,
      start_time=excluded.start_time, status=excluded.status, deleted=excluded.deleted, synced_at=now();`;
